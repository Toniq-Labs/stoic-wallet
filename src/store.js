...
function saveDb(newState) {
  var updatedDb = [
    [],
    newState.addresses,
    [newState.currentPrincipal, newState.currentAccount, newState.currentToken],
    DBVERSION,
  ];
  var loadedPrincipals = [];
  ...
  switch (action.type) {
      ...
      return saveDb({
        ...
        principals: state.principals.map((principal, i) => {
          if (i === state.currentPrincipal) {
            return {
              ...principal,
              apps: [...principal.apps, action.payload.app],
            };
          } else {
            return principal;
          }
      ...
      return saveDb({
        ...
        principals: state.principals.map((principal, i) => {
          if (i === state.currentPrincipal) {
            return {
              ...principal,
              neurons: [...principal.neurons, action.payload.neuron],
            };
          } else {
            return principal;
          }
    ...
    case 'deletewallet':
      return saveDb({
        ...state,
        currentPrincipal:
          state.currentPrincipal > action.payload.index
            ? state.currentPrincipal - 1
    ...
    case 'addwallet': //TODO
      if (state.principals.some(e => e.identity.principal === action.payload.identity.principal))
        return state;
    ...
    case 'account/token/add':
      if (
        state.principals[state.currentPrincipal].accounts[state.currentAccount].tokens.some(
          e => e.id === action.payload.metadata.id,
        )
      )
        return state;
      return saveDb({
