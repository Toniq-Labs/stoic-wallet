...
export default function Neuron(props) {
  ...
  const formatDate = d => {
    ...
    if (d < (3600 * 24 * 7 * 52) / 12)
      return _formatDateHelper(Math.round(d / 3600 / 24 / 7), 'week');
    if (d < 3600 * 24 * 7 * 52)
      return _formatDateHelper(Math.round(d / 3600 / 24 / 7 / (52 / 12)), 'month');
    if (d < 3600 * 24 * 7 * 52 * 8)
      return _formatDateHelper(Math.round((d / 3600 / 24 / 7 / 52) * 10) / 10, 'year');
    return '8 years'; //max
  };
  return (
        ...
        <CardHeader
          action={
            <>
              ...
              <Menu
                ...
                onClose={closeMenu}
              >
                {props.neuron.data.state === 1 ? (
                  <MenuItem onClick={() => action(0)}>Start Dissolving</MenuItem>
                ) : (
                  ''
                )}
                {props.neuron.data.state === 2 ? (
                  <MenuItem onClick={() => action(1)}>Stop Dissolving</MenuItem>
                ) : (
                  ''
                )}
                {props.neuron.data.state === 3 && props.neuron.data.stake > 0n ? (
                  <MenuItem onClick={() => action(2)}>Disburse</MenuItem>
                ) : (
                  ''
                )}
                {props.neuron.data.maturity > 0n ? (
                  <MenuItem onClick={() => action(3)}>Spawn Rewards Neuron</MenuItem>
                ) : (
                  ''
                )}
          ...
          title={
            <Typography style={{fontSize: 14}} color={'secondary'}>
              {props.neuron.data.state === 1 ? (
                <>Delay set to {formatDate(Number(props.neuron.data.dissolve_delay))}</>
              ) : (
                ''
              )}
              {props.neuron.data.state === 2 ? (
                <>Unlocking in {formatDate(Number(props.neuron.data.dissolve_delay))}</>
              ) : (
                ''
              )}
              {props.neuron.data.state === 3 && props.neuron.data.stake === 0n ? (
          ...
          subheader={
            <Typography style={{fontSize: 14}} color={'primary'}>
              {'#' + props.neuron.id}
              <IconButton
                href={'https://icscan.io/neuron/' + props.neuron.id}
                target="_blank"
          ...
          <>
            ...
            >
              ...
              <Grid style={{height: '100%', textAlign: 'center'}} item md={4}>
                ...
                {props.neuron.data.state === 3 && props.neuron.data.stake === 0n ? (
                  <LockOpen style={{fontSize: 52}} />
                ) : (
                  ''
                )}
                {props.neuron.data.state === 3 && props.neuron.data.stake > 0n ? (
                  <AllInclusiveIcon style={{fontSize: 52}} />
                ) : (
                  ''
                )}
              </Grid>
