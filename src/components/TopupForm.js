    ...
    extjs
      ...
      .then(r => {
        return props.alert(
          ...
          'Your cycles have been minted and sent to your canister',
        );
      })
      .catch(e => {
        ...
        {step === 0 ? (
          <DialogContent>
            <DialogContentText style={{textAlign: 'center', fontWeight: 'bold'}}>
              Please enter the Canister ID and the amount of ICP you would like converted to cycles
              and added to the canister
            </DialogContentText>
            <>
              <TextField
                autoFocus
          ...
          <DialogContent>
            <DialogContentText style={{textAlign: 'center'}}>
              Please confirm that you are about to send{' '}
              <strong style={{color: 'red'}}>{amount} ICP</strong>
              <br />
              from <strong style={{color: 'red'}}>{compressAddress(props.address)}</strong>
              <br />
              to add ~{Math.round(rate * amount * 100, 2) / 100}T Cycles to{' '}
              <strong style={{color: 'red'}}>{compressAddress(to)}</strong>
              <br />
