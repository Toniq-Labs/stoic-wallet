...
export default function Marketplace(props) {
  ...
  const withdraw = async () => {
    try {
      ...
      props.alert("Sorry!", "There are no funds to be withdrawn");
      props.loader(false);
      error(e);
    };
  }
  ...
  if (allowedPrincipals.indexOf(identity.principal) < 0) return (<>IN TESTING</>);
    ...
    {listings.length === 0 ?
      <div style={styles.empty}>
        ...
        <Button onClick={() => {props.loader(true);refreshListings();}} variant="contained" color="primary">Refresh Listings</Button> <Button onClick={withdraw} variant="contained" color="primary">Withdraw Funds</Button>
      </div> :
      <>
        <div style={styles.empty}>
          <Typography paragraph style={{paddingTop:20,fontWeight:"bold"}} align="center">
