...
export class Secp256k1KeyIdentity extends SignIdentity {
    ...
    static fromParsedJson(obj) {
        ...
        return new Secp256k1KeyIdentity(Secp256k1PublicKey.fromDer(Buffer.from(publicKeyDer, 'hex')), PrivateKey.fromDer(Buffer.from(privateKeyDer, "hex")));
    }
    static fromJSON(json) {
        ...
        if (Array.isArray(parsed)) {
            ...
            const pk = publicKey
                ? Secp256k1PublicKey.fromRaw(Buffer.from(new Uint8Array(publicKey.data)))
                : Secp256k1PublicKey.fromDer(Buffer.from(new Uint8Array(_publicKey.data)));
            if (publicKey && secretKey && secretKey.data) {
                return new Secp256k1KeyIdentity(pk, Buffer.from(new Uint8Array(secretKey.data)));
            }
            else if (_publicKey && _privateKey && _privateKey.data) {
                return new Secp256k1KeyIdentity(pk, Buffer.from(new Uint8Array(_privateKey.data)));
            }
        }
     ...
     */
    toJSON() {
        return [(this._publicKey.toDer()).toString('hex'), (stringToBytes(this._privateKey.toDer())).toString('hex')];
    }
    /**
    ...
    async sign(challenge) {
        const blob = challenge instanceof Buffer
            ? challenge
            : Buffer.from(new Uint8Array(challenge));
        ...
        return Buffer.from(new Uint8Array([...Array.from(new Uint8Array(ra)), ...Array.from(new Uint8Array(sa))]));
    }
}
