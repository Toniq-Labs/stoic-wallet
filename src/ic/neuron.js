...
class ICNeuron {
  ...
  async topup(from_sa, amount) {
    ...
    var memo = await rosettaApi
      .getTransactionsByAccount(this.data.address)
      .then(rs => Number(rs[0].memo));
    args = {
      controller: [],
      memo: memo,
    };
  ...
  async setDissolveTime(seconds, error) {
    //not really needed
    var commandArgs = {
      operation: [
        {
...
const NeuronManager = {
  ...
  getData: async (neuronid, id) => {
    ...
    if (ns.full_neurons.length === 1) {
      ...
      ndata['operator'] = true;
      ndata['address'] = principalToAccountIdentifier(
        GOVERNANCE_CANISTER_ID,
        ns.full_neurons[0].account,
      );
    }
    return ndata;
  },
